...
const int canonSpecs[6][20] =  {{  29,  32 ,  23 ,  36 ,  20 ,   37 ,   13 , 13  ,  15 ,  36 ,  11  ,   18 , 36   ,  13  , 35  , 15  , 29 ,   32 ,  20 ,   24},      // left-right lowest canon
                                ...
                                { 132,  98 ,  90 , 120 , 132 ,  97  ,   83 ,  92 ,  120,  92 ,   97 ,  131 ,  102 ,  105 , 105 , 105 , 95 ,  131 ,  95 , 120}};      // height highest canon
/*
...
void setup() {
  ...
  Serial.println("WHAT IS THE CHARGING STATUS : ");
  bool getChargingStatus = false;
  while(getChargingStatus == false) {
    recvWithEndMarker();
    showNewNumber();
    if(dataNumber != 0) {
      if(dataNumber == 1) {
...
void loop() {
  if(chargingStatus == 1) {
    ...
    for(int canon = 0; canon <= 2; canon++) {
      ...
      while(distanceLeft < distanceLeftNeeded - generalOffset || distanceLeft > distanceLeftNeeded + generalOffset) {
        
        if(distanceLeft < distanceLeftNeeded - generalOffset) {
          for(int i = 0; i < 8; i++) {
      ...
      while(pitch > angleNeeded + generalAngleOffset || pitch < angleNeeded - generalAngleOffset) {
        
        if(pitch > angleNeeded + generalAngleOffset) {
          for(int i = 0; i < 8; i++) {
            digitalWrite(SR[3][0], LOW);
